...
def main():
    ...
    for p in board.get("picks",[]):
        ...
        items.append({"signal":f'{p["match"]} — {p["market"]}',"grade":p.get("grade") or "UNKNOWN","suggested_units":(p.get("max_units") or 0) if passes else 0,"max_entry":f'Edge≥{gate["min_edge_pp"]:.0%}, EV≥{gate["min_ev"]:.0%}',"kalshi_ticker":ticker,"sportsbook_consensus":c.get("consensus_probability"),"model_vs_consensus_pp":gap,"status":status})
        snap.append(q)
