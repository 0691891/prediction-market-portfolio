...
def predict(history,home,away,asof,half,shrink,ha):
    # CSV dates have no kickoff hour: exclude every same-calendar-day result.
    # Otherwise an earlier CSV row on Saturday leaks into another Saturday fixture.
    history=[m for m in history if m[0].date()<asof.date()]
