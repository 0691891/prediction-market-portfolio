...
def main():
    ...
    for p in board.get('picks',[]):
        ...
        kickoff=ts(p.get('kickoff_utc') or b.get('commence_time')); mins=(kickoff-now).total_seconds()/60 if kickoff else None
        row={'snapshot_utc':now.isoformat(),'snapshot_hour_utc':hour.isoformat(),'pick_id':pid,'competition':p.get('competition'),'match':p.get('match'),'market':p.get('market'),'kickoff_utc':p.get('kickoff_utc') or b.get('commence_time'),'minutes_to_kickoff':round(mins,1) if mins is not None else None,'bucket':label(mins),'my_fair':p.get('fair_probability') if m.get('status')=='MODELLED' else None,'fair_source':p.get('fair_source'),'sportsbook_probability':b.get('consensus_probability'),'sportsbook_book_count':b.get('book_count'),'kalshi_probability':k.get('market_probability'),'kalshi_liquidity_usd':k.get('liquidity_usd'),'model_status':m.get('status'),'football_intelligence_status':i.get('status')}
