...
def main():
    ...
    for t in trades:
        pid=t.get('pick_id'); entry=t.get('entry_probability'); close_row=cby.get(pid) if pid else None
        sportsbook_close=close_row.get('sportsbook_probability') if close_row else None; kalshi_close=close_row.get('kalshi_probability') if close_row else None
        if entry is None and t.get('entry_decimal_odds'):entry=implied_from_decimal(t.get('entry_decimal_odds'))
        ref=kalshi_close if kalshi_close is not None else sportsbook_close
        clv=(ref-entry) if (ref is not None and entry is not None) else None
        trade_rows.append({'trade_id':t.get('trade_id'),'pick_id':pid,'position':t.get('position'),'entry_probability':entry,'closing_kalshi_probability':kalshi_close,'closing_sportsbook_probability':sportsbook_close,'closing_reference_probability':ref,'clv_probability_points':round(clv,4) if clv is not None else None,'status':'OK' if clv is not None else 'INSUFFICIENT_POINT_IN_TIME_DATA'})
    # Signal CLV: earliest stored snapshot vs closest pre-kickoff snapshot.
    ...
    for pid,c in cby.items():
        e=earliest.get(pid); 
        if not e:continue
        rows={'pick_id':pid,'match':c.get('match'),'market':c.get('market'),'entry_snapshot_utc':e.get('snapshot_utc'),'closing_snapshot_utc':c.get('snapshot_utc'),'entry_kalshi_probability':e.get('kalshi_probability'),'closing_kalshi_probability':c.get('kalshi_probability'),'entry_sportsbook_probability':e.get('sportsbook_probability'),'closing_sportsbook_probability':c.get('sportsbook_probability')}
        if e.get('kalshi_probability') is not None and c.get('kalshi_probability') is not None:rows['kalshi_signal_clv_pp']=round(c['kalshi_probability']-e['kalshi_probability'],4)
        if e.get('sportsbook_probability') is not None and c.get('sportsbook_probability') is not None:rows['sportsbook_signal_clv_pp']=round(c['sportsbook_probability']-e['sportsbook_probability'],4)
        signal_rows.append(rows)
    valid=[x['clv_probability_points'] for x in trade_rows if x.get('clv_probability_points') is not None]
    save('clv.json',{'updated_utc':now,'definition':'For a YES-side position, positive CLV means the chosen outcome closed at a higher implied probability than entry. Trade CLV prefers Kalshi close, falling back to de-vig sportsbook consensus.','trade_clv':trade_rows,'signal_clv':signal_rows,'summary':{'tracked_trades':len(valid),'mean_trade_clv_pp':round(sum(valid)/len(valid),4) if valid else None},'limitations':'Historical trades without pick_id + entry_probability cannot be assigned CLV retroactively without evidence.'})
